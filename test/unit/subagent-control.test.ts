...
describe("subagent control attention state", () => {
	...
	it("marks non-exempt open tools for attention at the active threshold", () => {
		...
		assert.equal(shouldEmitOpenToolAttention({ config: defaults, currentTool: "bash", currentToolStartedAt: 0, lastActivityAt: 240_000, now: 240_000 }), false);
		assert.equal(shouldEmitOpenToolAttention({ config: defaults, currentTool: "bash", currentToolStartedAt: 0, lastActivityAt: 1, now: 240_001 }), true);
